...
as a git **submodule** at `../services/docker-mailserver` (upstream:
...
- `mailserver.env` — the env config for the `mailserver` service
  (`compose.yml` -> `env_file: ./docker/mailserver/mailserver.env`). This is the
...
#    edit docker/appflowy/.env -> GOTRUE_SMTP_* and APPFLOWY_MAILER_SMTP_*
...
- `docker/mailserver/mailserver.env` -> `SSL_DOMAIN=`, `POSTMASTER_ADDRESS=`
- `docker/appflowy/.env` -> `GOTRUE_SMTP_HOST`, `APPFLOWY_MAILER_SMTP_HOST`, etc.
